...
import { Send, Bot, User, Loader2, Volume2, Mic, MicOff, VolumeX, X, Headphones } from 'lucide-react'
...
export default function ChatInterface({ workspaceId }: { workspaceId: number }) {
    ...
    const fetchSentenceAudio = async (text: string) => {
        if (audioBufferRef.current.has(text)) return audioBufferRef.current.get(text)
        try {
            const res = await fetch(`http://localhost:8000/generate/narration?text=${encodeURIComponent(text)}&voice=af_bella`)
    ...
    const playNextInQueue = useCallback(async (onFinalComplete?: () => void) => {
        if (queueRef.current.length === 0) {
            ...
            // Cleanup buffer
            audioBufferRef.current.forEach(url => URL.revokeObjectURL(url))
        ...
        queueRef.current.slice(0, 2).forEach(text => fetchSentenceAudio(text))
    ...
    const startListeningRef = useRef<((isConversational?: boolean) => void) | null>(null)

    const handleVoiceConversation = useCallback(async (text: string) => {
        if (!text.trim()) {
    ...
    const startListening = useCallback((isConversational = false) => {
            ...
            <div className="p-4 border-b bg-white flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <Button
                        ...
                        className={cn(
                            "text-xs gap-2",
                            autoPlay ? "text-blue-600" : "text-slate-400"
                        )}
                    >
                ...
                <div className="space-y-4 max-w-3xl mx-auto">
                    ...
                    {voiceTranscript && isListening && (
                        <div className="flex gap-3 flex-row-reverse">
                            ...
                            <div className="p-3 rounded-lg text-sm max-w-[80%] bg-blue-600/50 text-white rounded-tr-none italic animate-pulse">
                                {voiceTranscript}...
                            </div>
                        </div>
                    )}
                    {loading && (
                        <div className="flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="bg-white border shadow-sm p-3 rounded-lg rounded-tl-none flex items-center gap-2">
            ...
            <div className="p-4 bg-white border-t">
                ...
                >
                    ...
                    <Button
                        ...
                        className={cn(isListening && "bg-red-50 text-red-600 border-red-200 animate-pulse")}
                        disabled={loading}
                    >
                ...
                <div className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur-xl flex flex-col items-center justify-between p-8 text-white animate-in fade-in duration-300">
                    <div className="w-full flex justify-between items-center">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="text-sm font-medium">AI Tutor Voice Mode</span>
                        </div>
                        <Button
                            variant="ghost"
                            size="icon"
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center gap-6 w-full max-w-2xl text-center min-h-0">
                        ...
                        <div className="relative shrink-0 scale-90 sm:scale-100">
                            <div className={cn(
                                ...
                                isSpeaking ? "scale-110 shadow-[0_0_80px_rgba(37,99,235,0.4)]" : "scale-100",
                                isListening ? "shadow-[0_0_100px_rgba(37,99,235,0.6)]" : ""
                            )}>
                                <div className={cn(
                                    ...
                                    {isMuted ? <MicOff className="w-8 h-8 sm:w-12 sm:h-12" /> : <Mic className="w-8 h-8 sm:w-12 sm:h-12" />}
                                </div>
                        ...
                        <div className="w-full flex-1 min-h-0 flex flex-col justify-center gap-4">
                            <div className="max-h-[30vh] sm:max-h-[40vh] overflow-y-auto px-4 custom-scrollbar">
                                <p className={cn(
                                    ...
                                    {isSpeaking ? voiceResponse : isListening ? (voiceTranscript || "Listening...") : "I'm ready"}
                                </p>
                            </div>

                            {voiceTranscript && isListening && (
                                <p className="text-slate-400 text-xs sm:text-sm italic px-8">&quot;{voiceTranscript}&quot;</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-xs flex justify-around items-center bg-white/5 rounded-full p-2 backdrop-blur-md">
                        <Button
                            ...
                            className={cn(
                                ...
                                isMuted ? "bg-red-500/20 text-red-500 hover:bg-red-500/30" : "text-white hover:bg-white/10"
                            )}
                        >
