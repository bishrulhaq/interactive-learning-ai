...
export default function StudyPage() {
    ...
    const fetchWorkspace = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            ...
            const llmProvider = currentWorkspace?.llm_provider || appSettings.llm_provider
            const embedProvider = currentWorkspace?.embedding_provider || appSettings.embedding_provider
            const apiKey = appSettings.openai_api_key
            const ollamaUrl = currentWorkspace?.ollama_base_url || appSettings.ollama_base_url
    ...
    const embeddingProvider = workspace?.embedding_provider || globalSettings?.embedding_provider || 'openai'
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isImage && !hasOpenAiKey) {
            setUploadError('Image uploads currently require OpenAI Vision (API key). Add your key in Settings or upload a PDF/Word/PPT instead.')
        ...
        try {
            ...
            if (!axiosErr.response) {
                ...
                setUploadError(`Could not reach backend at ${backend}. Is it running?${msg}`)
                return
            }
    ...
    const handleDeleteDocument = async (docId: number) => {
        if (!confirm('Are you sure you want to delete this document? All associated learning content will remain, but chat search will no longer include this document.')) return
        try {
        ...
        <div className="flex h-screen overflow-hidden bg-slate-100 font-sans relative">
            ...
            <div className="w-1/2 p-4 flex flex-col gap-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            {workspace && (
                                <div className="flex items-center gap-2 mt-1">
                                    <Badge variant="outline" className="text-[10px] py-0 px-2 bg-blue-50/50 text-blue-600 border-blue-100 flex items-center gap-1">
                                        <Sparkles className="w-3 h-3" />
                                        {workspace.llm_provider || globalSettings?.llm_provider || 'openai'}: {workspace.llm_model || globalSettings?.openai_model || 'gpt-4o'}
                                    </Badge>
                                    <Badge variant="outline" className="text-[10px] py-0 px-2 bg-indigo-50/50 text-indigo-600 border-indigo-100 flex items-center gap-1">
                                        <Layers className="w-3 h-3" />
                                        {workspace.embedding_provider || globalSettings?.embedding_provider || 'openai'}: {(workspace.embedding_model || globalSettings?.embedding_model || 'text-embedding-3-small').split('/').pop()}
                                    </Badge>
                                </div>
                            )}
                        </div>
                ...
                <Card className="flex-1 bg-white overflow-hidden shadow-sm border-slate-200 flex flex-col">
                    ...
                    {processingCount > 0 && (
                        <div className="bg-blue-50 border-b border-blue-100 p-3 flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-blue-700/80">
                                    This can take a few minutes. If you&apos;re using Hugging Face embeddings for the first time, the model may download and take longer.
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        {!workspace ? (
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="text-[11px] text-slate-400 mt-1">
                                    Note: Image uploads currently require OpenAI Vision (API key).
                                </p>
                            </div>
                        ) : (
                                ...
                                {workspace?.documents.map((doc) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col items-end gap-1 shrink-0">
                                            <div className="flex items-center gap-1">
                                                {doc.status === 'completed' && (
                                                    <div className={cn(
                                                        ...
                                                        (doc.embedding_model === (workspace.embedding_model || globalSettings?.embedding_model || 'text-embedding-3-small'))
                                                            ? "bg-slate-50 text-slate-500 border-slate-100"
                                                            : "bg-amber-50 text-amber-600 border-amber-100"
                                                    )}>
                                                        {doc.embedding_model?.split('/').pop() || 'unknown'}
                                                        {doc.embedding_model !== (workspace.embedding_model || globalSettings?.embedding_model || 'text-embedding-3-small') && (
                                                ...
                                                <Badge
                                                    ...
                                                    className={cn(
                                                        "text-[10px] h-5",
                                                        doc.status === 'completed' ? "bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-50" :
                                                            doc.status === 'failed' ? "bg-red-50 text-red-700 border-red-100" : "animate-pulse"
                                                    )}
                                                >
                                                    {doc.status}
                                                </Badge>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {doc.status !== 'completed' && doc.status !== 'pending' && doc.status !== 'processing' && (
                                                    <Button
                                                        size="icon"
                                                        variant="ghost"
                                                ...
                                                {doc.embedding_model !== (workspace.embedding_model || globalSettings?.embedding_model || 'text-embedding-3-small') && doc.status === 'completed' && (
                                                    <Button
                                                        size="icon"
                                                        variant="ghost"
                                            ...
                                            {doc.status === 'failed' && doc.error_message && (
                                                <div className="flex items-center gap-1 text-[9px] text-red-500 font-medium max-w-[120px] truncate" title={doc.error_message}>
                                                    <AlertCircle className="w-2 h-2" />
                                                    {doc.error_message}
                                                </div>
                                            )}
            ...
            <div className="w-1/2 bg-white border-l shadow-xl shadow-slate-200/50 flex flex-col relative">
                ...
                {workspace && workspace.documents.filter(d => d.status === 'completed').length > 0 && !isAiReady && (
