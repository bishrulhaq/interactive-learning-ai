...
def generate_lesson_plan(topic: str, db: Session) -> LessonPlan:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an expert educational content creator. Create a comprehensive lesson plan based strictly on the provided context."),
...
def generate_flashcards(topic: str, db: Session) -> FlashcardSet:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Create a set of 5-10 flashcards (Front/Back) based on the context to help a student learn the key concepts."),
        ("user", "Context: {context}\n\nTopic: {topic}\n\nGenerate flashcards:")
    ])
...
def generate_quiz(topic: str, db: Session) -> Quiz:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Create a 5-question multiple choice quiz based on the context."),
        ("user", "Context: {context}\n\nTopic: {topic}\n\nGenerate quiz:")
    ])
