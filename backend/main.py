...
from backend.models import Document, Message, GeneratedLesson, GeneratedFlashcard, GeneratedQuiz
...
def chat(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    # 2. Save User Message
    user_msg = Message(document_id=request.document_id, role="user", content=request.message)
    db.add(user_msg)
...
def get_chat_history(document_id: int, db: Session = Depends(get_db)):
    stmt = select(Message).filter(Message.document_id == document_id).order_by(Message.created_at)
